...
public class Mem_stsFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		EmpVO empVO = (EmpVO) session.getAttribute("empVO2");
...
//		String repo = req.getContextPath() + "";
		...
		if (empVO != null) {
			if (memVO == null && mem_no != null) {
				memVO2 = memSvc.getOneMem(mem_no);
				session.setAttribute("memVO2", memVO2);
			} else {
				memVO2 = memSvc.getOneMem("0000");
				session.setAttribute("memVO2", memVO2);
			}
